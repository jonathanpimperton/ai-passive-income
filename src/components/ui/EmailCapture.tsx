...
export default function EmailCapture({ toolSlug, toolName }: EmailCaptureProps) {
  ...
  if (status === 'success') {
    return (
      <div className="bg-accent-100 border border-accent-500/20 rounded-xl p-6 text-center">
        <svg
          className="w-8 h-8 text-accent-600 mx-auto mb-2"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
    ...
    <div className="bg-primary-50 border border-primary-200 rounded-xl p-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 px-4 py-2.5 text-sm border border-neutral-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"
        />
        <button
          type="submit"
